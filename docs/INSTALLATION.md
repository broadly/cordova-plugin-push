...
```bash
cordova plugin add @broadly/cordova-plugin-push
```
...
```bash
cordova plugin add github:broadly/cordova-plugin-push
```
...
```xml
<plugin name="@broadly/cordova-plugin-push" spec="3.0.0" />
```
...
When some other packages include `cordova-google-play-services` as a dependency, such as is the case with the `cordova-admob` and `cordova-plugin-analytics` plugins, it is impossible to also add the `@broadly/cordova-plugin-push`, for the following error will rise during the build process:
...
```log
Installing "@broadly/cordova-plugin-push" for ios
Failed to install '@broadly/cordova-plugin-push':Error: pod: Command failed with exit code 1
